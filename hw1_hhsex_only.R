...
hh_coef <- coef(mi_hh)
hh_se <- SE(mi_hh)
hh_pop <- as.numeric(coef(mi_hh_pop))
hh_df <- data.frame(hhsex = names(hh_coef), estimate = as.numeric(hh_coef), se = as.numeric(hh_se), pop = hh_pop)
write.csv(hh_df, file = 'c:/Users/takao/Documents/wu-seminar-distribution/hw1/hhsex_networth_by_group.csv', row.names = FALSE)
cat('Wrote hhsex results to hhsex_networth_by_group.csv\n')
print(hh_df)
